...
from animals_utils import get_sequence,get_atomic_clips,split_by_indices
...
from first_order_triangle_ode_animals import TriangleODE
...
class AnimalsODETrainer():
    ...
    def one_sequence(self,seq,primary_jacobians,faces,times,first_shape,poisson_solver,loss_args,train=True,test_shape=None,test_faces=None,pose=None,betas=None,full_sequence_pose=None,full_sequence_times=None,clip_num=None,split_indices=None,prev_pred_jacobians=None,prev_pred_last_def=None,no_gt=False):
        '''
    ...
    def compute_and_write_wks(self,training_args,data_args,test_args):
        ...
        for bix,data in enumerate(dataloader):
            poses,trans,betas,betas_limbs,offset,name = data
            sequence_pose,trans,betas,betas_limbs,offset = convert_to_float_tensors(poses,trans,betas,betas_limbs,offset)
            sequence,faces = get_sequence(sequence_pose,trans,betas,betas_limbs,offset,data_args.max_frames)
    ...
    def amass_one_epoch(self,optimizer,epochs,loss_args,training_args,test_args,data_args,epoch):
        ...
        for bix,data in enumerate(dataloader):
            ...
            sequence_pose,trans,betas,betas_limbs,offset = convert_to_float_tensors(poses,trans,betas,betas_limbs,offset)
            sequence,faces = get_sequence(sequence_pose,trans,betas,betas_limbs,offset,data_args.max_frames)
            sequence_pose = sequence_pose.squeeze()
            sequence_pose = sequence_pose.view(sequence_pose.size()[0],-1)
            betas = torch.cat([betas,betas_limbs],-1)
            ...
            for i,(clip,times,pose) in enumerate(zip(clips,clip_times,clip_poses)):
                shape_seq = clip
                #with torch.no_grad():
                _,_,_,_,primary_jacobians = self.primary.one_sequence(shape_seq,faces,times,first_shape,sequence_poisson_solver,loss_args,d3f=face_features,pose=pose,betas=betas,clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=seq_times)
                #primary_jacobians = sequence_poisson_solver.jacobians_from_vertices(wodmpl_clip).contiguous()
                seq_loss,jac_loss,def_shape,previous_pred_jacobians,previous_pred_last_def,clip_v2v = self.one_sequence(shape_seq,primary_jacobians,faces,times,first_shape,sequence_poisson_solver,loss_args,pose=pose,betas=betas,full_sequence_pose=sequence_pose,full_sequence_times=seq_times,clip_num=i,split_indices=split_indices,prev_pred_jacobians=previous_pred_jacobians,prev_pred_last_def=previous_pred_last_def)
    ...
    def def_trans(self,optimizer,epochs,loss_args,training_args,test_args,data_args):
        self.primary.preload(training_args.pose_weights,training_args.feature_weights,training_args.weight_path)
        self.jacobian_func.load_state_dict(torch.load(training_args.weight_path+'_jacobian'))
        self.attention.load_state_dict(torch.load(training_args.weight_path+'_attention'))
        #self.joints_encoder.load_state_dict(torch.load(training_args.weight_path+'_joints'))
        ...
        with torch.no_grad():
            ...
            for bix,data in enumerate(dataloader):
                ...
                poses,trans,_,_,offset,source_name = data

                test_seq_ix = 0
                for tbix,test_data in enumerate(test_dataloader):
                    _,_,test_betas,test_betas_limbs,test_offset,target_name = test_data
                    
                    given_source_name = source_name[0]
                    given_test_name = target_name[0]
                    ...
                    if not (given_source_name in test_args.src_names and given_test_name in test_args.tgt_names):
                        continue
                    sequence_pose,trans,test_betas,test_betas_limbs,offset = convert_to_float_tensors(poses,trans,test_betas,test_betas_limbs,offset)
                    test_sequence,faces = get_sequence(sequence_pose,trans,test_betas,test_betas_limbs,offset,data_args.max_frames)
                    sequence_pose = sequence_pose.squeeze()
                    sequence_pose = sequence_pose.view(sequence_pose.size()[0],-1)
                    test_betas = torch.cat([test_betas,test_betas_limbs],-1)
                    ...
                    _,source_clip_times,sequence_times,split_indices = get_atomic_clips(test_sequence,self.frame_length)
                    ...
                    feature_path = os.path.join(data_args.feature_dir,target_name[0]+".npy")
                    feature_np = np.load(feature_path)
                    ...
                    for i,(test_clip,times,source_pose) in enumerate(zip(test_clips,source_clip_times,clip_poses)):
                        _,_,_,_,primary_jacobians = self.primary.one_sequence(test_clip,faces,times,rest_shape,sequence_poisson_solver,loss_args,d3f=face_features,pose=source_pose,betas=test_betas,clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times)
                        trans_sequence,prev_pred_jacobians,prev_pred_last_def,v2v_errors = self.one_sequence(test_clip,primary_jacobians,faces,times,rest_shape,sequence_poisson_solver,loss_args,train=False,pose=source_pose,betas=test_betas,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,clip_num=i,split_indices=split_indices,prev_pred_jacobians=prev_pred_jacobians,prev_pred_last_def=prev_pred_last_def)
                        ...
                        start_frame += len(test_clip) 
                        clip_ix += 1
                        message = colored("Rendered {0:3d} of {1:3d}".format(clip_ix,len(test_clips)),'blue')
                        logger.info(message)
                    ...
                    if True: #test_args.sig:
                        for i,trans_sequence in enumerate(pred_seq):
                            ...
                            #display_utils.write_amass_sequence(out_dir,"",trans_sequence,faces,start_frame,transfer_color,"tgt",r90=False)
                            display_utils.write_amass_sequence(gt_out_dir,"",gt_sequence[i],faces,start_frame,transfer_color,"tgt",r90=False)
                            #display_utils.write_amass_sequence(out_dir,"",sk_clip,sk_faces,start_frame,source_color,"src")
                            start_frame+=len(trans_sequence)
                    
                    test_seq_ix += 1
                    seq_v2p_error /= len(test_clips)
                    seq_v2v_error /= len(test_clips)
    ...
    def def_trans_nonhuman(self,optimizer,epochs,loss_args,training_args,test_args,data_args):
        ...
        with torch.no_grad():
            ...
            for bix,data in enumerate(dataloader):
                #if source_seq_ix==1:break
                poses,trans,betas,betas_limbs,offset,source_name = data
                test_seq_ix = 0
                for tbix,test_data in enumerate(dataloader):
                    _,_,_,_,offset,test_name = data
                    ...
                    if not (given_source_name in test_args.src_names and given_test_name in test_args.tgt_names):
                        continue
                    sequence_pose,trans,betas,betas_limbs,offset = convert_to_float_tensors(poses,trans,betas,betas_limbs,offset)
                    test_sequence,faces = get_sequence(sequence_pose,trans,betas,betas_limbs,offset,data_args.max_frames)
                    sequence_pose = sequence_pose.squeeze()
                    sequence_pose = sequence_pose.view(sequence_pose.size()[0],-1)
                    test_betas = torch.cat([betas,betas_limbs],-1)
                    #source_sequence,test_sequence_wodmpl,test_sequence = align_feet(source_sequence,test_sequence_wodmpl,test_sequence)
                    rest_shape = nonhuman_vertices
                    source_clips,source_clip_times,sequence_times,split_indices = get_atomic_clips(test_sequence,self.frame_length)
                    test_clips,_,_ = split_by_indices(test_sequence,sequence_times,split_indices)
                    clip_poses,_,_ = split_by_indices(sequence_pose,sequence_times,split_indices)
                    ...
                    for i,(clip,test_clip,times,source_pose) in enumerate(zip(source_clips,test_clips,source_clip_times,clip_poses)):
                        _,primary_jacobians = self.primary.one_sequence(clip,nonhuman_faces,times,rest_shape,sequence_poisson_solver,loss_args,d3f=face_features,pose=source_pose,betas=test_betas,clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,train=False,no_gt=True)
                        trans_sequence,prev_pred_jacobians,prev_pred_last_def = self.one_sequence(clip,primary_jacobians,nonhuman_faces,times,rest_shape,sequence_poisson_solver,loss_args,train=False,pose=source_pose,betas=test_betas,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,clip_num=i,split_indices=split_indices,prev_pred_jacobians=prev_pred_jacobians,prev_pred_last_def=prev_pred_last_def,no_gt=True)
                        pred_seq.append(trans_sequence)
                        frame_ix += len(clip)
                    ...
                    if True: #test_args.sig:
                        for i,trans_sequence in enumerate(pred_seq):
                            ...
                            #display_utils.write_amass_sequence(gt_out_dir,"",trans_sequence,nonhuman_faces,start_frame,transfer_color,"tgt",r90=False)
                            #display_utils.write_amass_sequence(out_dir,"",sk_clip,sk_faces,start_frame,source_color,"src")
                            start_frame+=len(trans_sequence)
