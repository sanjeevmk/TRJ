...
from init import init_amass_sequence_data,init_posing_network,init_pointnet_network,init_transformer_network,init_animals_sequence_data
from init import init_jacobian_network_animals,init_joints_encoder_network,init_ours_wks_posing_network_animals,init_d3f_pointnet_network
#from spacetime import Method

import logging
import os
from animals_ode_train import AnimalsODETrainer 

def run(data_args,training_args,loss_args,test_args,log_args):
   dataset = init_animals_sequence_data(data_args.root,data_args.bodymodel,data_args.seq_length,data_args.max_frames)

   if test_args.test:
      test_dataset = init_animals_sequence_data(test_args.testdir,data_args.bodymodel,data_args.seq_length,data_args.max_frames) 

   posing_network = init_ours_wks_posing_network_animals()
   ...
   jacobian_network = init_jacobian_network_animals()
   ...
   if not test_args.test:
         method = AnimalsODETrainer(posing_network,pointnet_network,jacobian_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir)
         method.loop_epochs(optimizer,training_args.epochs,loss_args,training_args,test_args,data_args)
   else:
      method = AnimalsODETrainer(posing_network,pointnet_network,jacobian_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir,test_dataset=test_dataset)
      if test_args.mode == "nonhuman":
         ...
         method.def_trans(optimizer,training_args.epochs,loss_args,training_args,test_args,data_args)
