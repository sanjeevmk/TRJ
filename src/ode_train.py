...
from first_order_triangle_ode import TriangleODE
...
class OdeTrainer():
    ...
    def get_ode_solution_first_order(self,initial_delta,j0,primary_attention,prev_pred_attention,pose_enc,target_j,times,betas):
        num_faces = j0.size()[0]
        ...
        initial_tensor = torch.cat([num_faces_tensor,initial_delta.view(1,-1),j0.view(1,-1),primary_attention.view(1,-1),prev_pred_attention.view(1,-1),pose_enc.view(1,-1),target_j.view(1,-1),betas.view(1,-1)],1)
    ...
    def one_sequence(self,seq,primary_jacobians,faces,times,first_shape,poisson_solver,loss_args,train=True,test_shape=None,test_faces=None,pose=None,betas=None,full_sequence_pose=None,full_sequence_times=None,clip_num=None,split_indices=None,prev_pred_jacobians=None,prev_pred_last_def=None,no_gt=False):
        ...
        deformation_jacobians = self.get_ode_solution_first_order(first_def,first_j0,primary_attention,previous_attention,pose_enc,primary_jacobians[-1].contiguous(),times,betas)
    ...
    def amass_one_epoch(self,optimizer,epochs,loss_args,training_args,test_args,data_args,epoch):
        ...
        for bix,data in enumerate(dataloader):
            ...
            for i,(clip,times,pose) in enumerate(zip(clips,clip_times,clip_poses)):
                shape_seq = clip
                #with torch.no_grad():
                _,_,_,_,primary_jacobians,cn_feat = self.primary.one_sequence(shape_seq,faces,times,first_shape,sequence_poisson_solver,loss_args,pose=pose,betas=betas[0,0,0,:],clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=seq_times)
                #primary_jacobians = sequence_poisson_solver.jacobians_from_vertices(wodmpl_clip).contiguous()
                seq_loss,jac_loss,_,previous_pred_jacobians,previous_pred_last_def,clip_v2v = self.one_sequence(shape_seq,primary_jacobians,faces,times,first_shape,sequence_poisson_solver,loss_args,pose=pose,betas=betas[0,0,0,:],full_sequence_pose=sequence_pose,full_sequence_times=seq_times,clip_num=i,split_indices=split_indices,prev_pred_jacobians=previous_pred_jacobians,prev_pred_last_def=previous_pred_last_def)
            ...
            if len(batch_codes) > 0:
                if len(batch_codes) > 1:
                    ...
                    optimizer.zero_grad() ; self.jacobian_func.zero_grad() ; self.posing_func.zero_grad() ; self.attention.zero_grad() ; self.pointnet_func.zero_grad()
    ...
    def loop_epochs(self,optimizer,epochs,loss_args,training_args,test_args,data_args):
        ...
        #exit()
        #self.primary.preload(training_args.pose_weights,training_args.feature_weights,training_args.preload_path)
        ...
        with torch.no_grad():
            ...
            for bix,data in enumerate(dataloader):
                ...
                for tbix,test_data in enumerate(test_dataloader):
                    ...
                    for i,(clip,wodmpl_test_clip,test_clip,times,source_pose) in enumerate(zip(source_clips,wodmpl_test_clips,test_clips,source_clip_times,clip_poses)):
                        _,_,_,_,primary_jacobians,cn_feat = self.primary.one_sequence(clip,faces,times,rest_shape,sequence_poisson_solver,loss_args,pose=source_pose,betas=betas[0,0,0,:],clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times)
                        #primary_jacobians = sequence_poisson_solver.jacobians_from_vertices(wodmpl_test_clip).contiguous()
                        trans_sequence,prev_pred_jacobians,prev_pred_last_def,v2v_errors = self.one_sequence(clip,primary_jacobians,faces,times,rest_shape,sequence_poisson_solver,loss_args,train=False,pose=source_pose,betas=betas[0,0,0,:],full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,clip_num=i,split_indices=split_indices,prev_pred_jacobians=prev_pred_jacobians,prev_pred_last_def=prev_pred_last_def)
    ...
    def def_trans_nonhuman(self,optimizer,epochs,loss_args,training_args,test_args,data_args):
        ...
        #center = torch.mean(nonhuman_vertices,dim=0,keepdim=True)
        #nonhuman_vertices -= center
            ...
            for bix,data in enumerate(dataloader):
                ...
                for tbix,test_data in enumerate(test_dataloader):
                    ...
                    for i,(clip,test_clip,times,source_pose) in enumerate(zip(source_clips,test_clips,source_clip_times,clip_poses)):
                        ...
                        #exit()
                        _,primary_jacobians = self.primary.one_sequence(clip,nonhuman_faces,times,rest_shape,sequence_poisson_solver,loss_args,pose=source_pose,betas=betas[0,0,0,:],clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,train=False,no_gt=True)
                        trans_sequence,prev_pred_jacobians,prev_pred_last_def = self.one_sequence(clip,primary_jacobians,nonhuman_faces,times,rest_shape,sequence_poisson_solver,loss_args,train=False,pose=source_pose,betas=betas[0,0,0,:],full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,clip_num=i,split_indices=split_indices,prev_pred_jacobians=prev_pred_jacobians,prev_pred_last_def=prev_pred_last_def,no_gt=True)
                        pred_seq.append(trans_sequence)
                        frame_ix += len(clip)
                    ...
                    if True: #test_args.sig:
                        for i,trans_sequence in enumerate(pred_seq):
                            #display_utils.write_amass_sequence(out_dir,"",trans_sequence,faces,start_frame,transfer_color,"tgt",soft_errors=soft_v2v[start_frame:start_frame+len(trans_sequence)]) #v2v_errors=per_vertex_acc_errors[start_frame:start_frame+len(trans_sequence)])
                            display_utils.write_amass_sequence(out_dir,"",trans_sequence,nonhuman_faces,start_frame,transfer_color,"tgt")
