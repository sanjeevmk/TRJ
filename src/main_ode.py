...
from init import init_jacobian_network,init_joints_encoder_network
#from spacetime import Method
import logging
import os
from ode_train import OdeTrainer
...
def run(data_args,training_args,loss_args,test_args,log_args):
   ...
   jacobian_network = init_jacobian_network()
   ...
   if not test_args.test:
         method = OdeTrainer(posing_network,pointnet_network,jacobian_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir)
         method.loop_epochs(optimizer,training_args.epochs,loss_args,training_args,test_args,data_args)
   else:
      method = OdeTrainer(posing_network,pointnet_network,jacobian_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir,test_dataset=test_dataset)
