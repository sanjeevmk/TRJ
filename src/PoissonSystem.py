...
if __name__ == "__main__":
    ...
    import igl,os
    '''
    print(os.path.exists(obj_path))
    V,F,G = igl.readOBJ(obj_path) #,process=False)

    for i, group in enumerate(G):
        # Extract the vertices of the group
        group_V = V[group]

        # Extract the faces of the group and re-index them
        group_F = np.zeros_like(group, dtype=np.int32)
        for j, f in enumerate(F):
            for k in range(3):
                if f[k] in group:
                    group_F[j, k] = np.where(group == f[k])[0][0]
        group_poisson_matrices = poisson_system_matrices_from_mesh(np.array(group_V),np.array(group_F))
        group_poisson_solver = group_poisson_matrices.create_poisson_solver().to('cuda')
        test_inp = torch.randn(1,len(group_F),3,3).float().cuda()
        solution = group_poisson_solver.solve_poisson(test_inp)
        print(solution.size())
    exit()
    '''
    _mesh = trimesh.load(obj_path,process=False)
    print(_mesh.vertices.shape,_mesh.faces.shape)
    poisson_matrices = poisson_system_matrices_from_mesh(np.array(_mesh.vertices),np.array(_mesh.faces))
    #poisson_matrices = poisson_system_matrices_from_mesh(np.array(V),np.array(F))
