...
from posing_d3f_signature import D3FPosingTrainerSignature as Primary
...
class D3FOdeTrainer():
    def __init__(self,posing_func,pointnet_func,jacobian_func,transformer_network,joints_encoder,seq_dataset,batch,frame_length,logdir,test_dataset=None):
        ...
        self.primary = Primary(posing_func,pointnet_func,self.joints_encoder,seq_dataset,batch,frame_length,logdir)

    def batch_backprop(self,optimizer,batch_seq_loss,batch_jac_loss,batch_joint_loss):
        loss = batch_seq_loss + 0.1*batch_jac_loss + batch_joint_loss
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        ...
        self.joints_encoder.zero_grad()
        batch_seq_loss = 0.0 ; batch_jac_loss = 0.0 ; batch_joint_loss = 0.0

        return batch_seq_loss,batch_jac_loss,batch_joint_loss
    ...
    def amass_one_epoch(self,optimizer,epochs,loss_args,training_args,test_args,data_args,epoch):
        ...
        for bix,data in enumerate(dataloader):
            ...
            mean_seq_joint_loss = 0.0
            clips,clip_times,untranslated_times,seq_times,split_indices = get_atomic_clips(sequence,self.frame_length,fps,fixed_time=training_args.fixed_time)
            ...
            for i,(clip,times,unt_times,pose) in enumerate(zip(clips,clip_times,untranslated_times,clip_poses)):
                shape_seq = clip
                #with torch.no_grad():
                _,_,joint_loss,_,_,primary_jacobians = self.primary.one_sequence(shape_seq,faces,unt_times,first_shape,sequence_poisson_solver,loss_args,d3f=face_features,pose=pose,betas=betas[0,0,0,:],clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=seq_times)
                ...
                batch_joint_loss += (joint_loss)
                mean_seq_seq_loss += seq_loss.detach().item()
                mean_seq_jac_loss += jac_loss.detach().item()
                mean_seq_joint_loss += joint_loss.detach().item()
                mean_sequence_v2v += clip_v2v.mean().cpu().detach().item()
                previous_times = times
                now = timer()

                if i%batch==0:
                    batch_seq_loss,batch_jac_loss,batch_joint_loss = self.batch_backprop(optimizer,batch_seq_loss,batch_jac_loss,batch_joint_loss)
                    batch_codes = []
                ...
                if  clip_ix % log_every == 0:
                    message = colored("Seq Loss: {0:2.6f} Jac loss: {1:2.6f} Joint loss: {2:2.5f} Clip: {3:4d} of {4:4d}"
                    .format(mean_seq_seq_loss/(clip_ix+1),mean_seq_jac_loss/(clip_ix+1),mean_seq_joint_loss/(clip_ix+1),clip_ix,len(clips)),'cyan')
                    logger.info(message)
            ...
            mean_joint_loss += (mean_seq_joint_loss/len(clips))
            ...
            if len(batch_codes) > 0:
                if len(batch_codes) > 1:
                    ...
                    optimizer.zero_grad() ; self.jacobian_func.zero_grad() ; self.posing_func.zero_grad() ; self.attention.zero_grad() ; self.joints_encoder.zero_grad()
        ...
        mean_joint_loss /= num_sequences
        mean_seq_v2v /= num_sequences

        return mean_seq_loss,mean_jac_loss,mean_joint_loss,mean_seq_v2v
    ...
    def loop_epochs(self,optimizer,epochs,loss_args,training_args,test_args,data_args):
        ...
        for i in range(1000):
            ...
            ep_seq_loss,ep_jac_loss,ep_joint_loss,ep_v2v = self.amass_one_epoch(optimizer,epochs,loss_args,training_args,test_args,data_args,i)
            #scheduler.step()
            ...
            if ep_joint_loss < best_joint_loss:
                best_joint_loss = ep_joint_loss
            if ep_v2v < best_v2v:
                best_v2v = ep_v2v

            message = colored("Best Ep: {0:3d} Best Seq Loss: {1:2.6f} Best Jac loss: {2:2.6f} Best Joint loss: {3:2.6f} Best v2v: {4:2.6f}"
            .format(best_epoch,best_seq_loss,best_jac_loss,best_joint_loss,best_v2v),'green')
            logger.info(message)
        ...
        with torch.no_grad():
            ...
            for bix,data in enumerate(dataloader):
                ...
                for tbix,test_data in enumerate(test_dataloader):
                    ...
                    wodmpl_test_sequence,faces = get_sequence(root_zero,pose_body,pose_hand,trans,test_betas,dmpls,test_gender,data_args.max_frames,use_dmpl=False)
            ...
            for bix,data in enumerate(dataloader):
                ...
                for tbix,test_data in enumerate(test_dataloader):
                    ...
                    for i,(clip,test_clip,times,source_pose,g_rotation,g_trans) in enumerate(zip(source_clips,test_clips,source_clip_times,clip_poses,clip_rotations,clip_translations)):
                        #if i==3:
                        #    break
                        _,primary_jacobians = self.primary.one_sequence(clip,nonhuman_faces,times,rest_shape,sequence_poisson_solver,loss_args,d3f=face_features,pose=source_pose,betas=test_betas[0,0,0,:],clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,train=False,no_gt=True)
