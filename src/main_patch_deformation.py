...
import logging
import os
from patch_deform_train import PatchTrainer 
...
def run(data_args,training_args,loss_args,test_args,log_args):
   ...
   optimizer = init_optimizers([vertex_deformation_network,transformer_network],training_args.lr)

   if not test_args.test:
         method = PatchTrainer(posing_network,pointnet_network,vertex_deformation_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir)
         method.loop_epochs(optimizer,training_args.epochs,loss_args,training_args,test_args,data_args)
   else:
      method = PatchTrainer(posing_network,pointnet_network,vertex_deformation_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir,test_dataset=test_dataset)
