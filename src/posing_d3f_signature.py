...
class D3FPosingTrainerSignature():
    ...
    def one_sequence(self,seq,faces,times,first_shape,poisson_solver,loss_args,train=True,test_shape=None,test_faces=None,pose=None,betas=None,clip_num=None,full_sequence_pose=None,full_sequence_times=None,d3f=None,no_gt=False):
        ...
        if train:
            ...
            return seq_loss_shape,jac_loss,deformed_shape,v2v_errors,pred_jacobians
        else:
            #target_shape,_,_ = shape_normalization_transforms_pytorch(seq)
            if not no_gt:
    ...
    def amass_one_epoch(self,optimizer,epochs,loss_args,training_args,test_args,data_args,epoch):
        ...
        for bix,data in enumerate(dataloader):
            ...
            if not training_args.root_zero:
                ...
                wodmpl_sequence,_ = get_sequence(root_zero,pose_body,pose_hand,trans,betas,dmpls,gender,data_args.max_frames,use_dmpl=True)
            ...
            for i,(wodmpl_clip,primary_mean_clip,times,pose) in enumerate(zip(wodmpl_clips,primary_mean_clips,clip_times,clip_poses)):
                ...
                wodmpl_seq_loss,wodmpl_jac_loss,_,v2v_errors,_ = self.one_sequence(wodmpl_shape_seq,faces,times,wodmpl_first_shape,current_beta_poisson_solver,loss_args,d3f=face_features,pose=pose,betas=zero_betas[0,0,0,:],full_sequence_pose=sequence_pose,full_sequence_times=seq_times)
        ...
        with torch.no_grad():
            ...
            for bix,data in enumerate(dataloader):
                ...
                for tbix,test_data in enumerate(test_dataloader):
                    ...
                    root_zero = torch.zeros(root_orient.size()).float().cuda()
                    root_orient,pose_body,pose_hand,trans,betas,test_betas,dmpls,fps = convert_to_float_tensors(root_orient,pose_body,pose_hand,trans,betas,test_betas,dmpls,fps)
                    source_sequence,faces = get_sequence(root_zero,pose_body,pose_hand,trans,betas,dmpls,gender,data_args.max_frames,use_dmpl=False)

                    sequence_pose = torch.cat([root_zero.squeeze(),pose_body.squeeze(),pose_hand.squeeze()],-1)
                    test_sequence,faces = get_sequence(root_zero,pose_body,pose_hand,trans,test_betas,dmpls,test_gender,data_args.max_frames,use_dmpl=False)
                    ...
                    for i,(clip,test_clip,times,source_pose) in enumerate(zip(source_clips,test_clips,source_clip_times,clip_poses)):
                        trans_sequence,v2v_errors = self.one_sequence(clip,faces,times,rest_shape,sequence_poisson_solver,loss_args,d3f=face_features,train=False,pose=source_pose,betas=test_betas[0,0,0,:],clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times)
            ...
            for bix,data in enumerate(dataloader):
                ...
                for tbix,test_data in enumerate(test_dataloader):
                    ...
                    for i,(clip,test_clip,times,source_pose,g_rotation,g_trans) in enumerate(zip(source_clips,test_clips,source_clip_times,clip_poses,clip_rotations,clip_translations)):
                        trans_sequence,primary_jacobians = self.one_sequence(clip,nonhuman_faces,times,rest_shape,sequence_poisson_solver,loss_args,d3f=face_features,pose=source_pose,betas=test_betas[0,0,0,:],clip_num=i,full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,train=False,no_gt=True)
