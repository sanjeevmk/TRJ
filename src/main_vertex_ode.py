...
import logging
import os
from vertex_ode_train import VertexOdeTrainer
...
def run(data_args,training_args,loss_args,test_args,log_args):
   ...
   pointnet_network = init_vertex_pointnet_network()
   ...
   vertex_deformation_network = init_vertex_deformation_network()
   ...
   optimizer = init_optimizers([vertex_deformation_network,pointnet_network],training_args.lr)

   if not test_args.test:
         method = VertexOdeTrainer(posing_network,pointnet_network,vertex_deformation_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir)
         method.loop_epochs(optimizer,training_args.epochs,loss_args,training_args,test_args,data_args)
   else:
      method = VertexOdeTrainer(posing_network,pointnet_network,vertex_deformation_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir,test_dataset=test_dataset)
