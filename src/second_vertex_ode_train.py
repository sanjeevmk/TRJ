...
from second_order_vertex_ode import VertexODE 
from util_networks import CustomPositionalEncoding
from posing_pretrain import PosingTrainer as Primary
from torch_scatter import scatter
from vertex_deformation_network import VertexVelocityIntegrator

class VertexOdeTrainer():
    def __init__(self,posing_func,pointnet_func,vertex_acceleration_func,transformer_network,joints_encoder,seq_dataset,batch,frame_length,logdir,test_dataset=None):
        ...
        self.vertex_acceleration_func = vertex_acceleration_func 
        self.joints_encoder = joints_encoder
        self.velocity_integrator = VertexVelocityIntegrator()
        self.ode_evaluator = VertexODE(self.vertex_acceleration_func,self.velocity_integrator)
    ...
    def batch_backprop(self,optimizer,batch_seq_loss):
        ...
        self.vertex_acceleration_func.zero_grad()
        batch_seq_loss = 0.0
    ...
    def get_ode_solution_second_order(self,initial_velocity,v0,primary_attention,times,betas):
        num_verts = v0.size()[0]
        primary_attention = primary_attention.clone().detach()
        num_verts_tensor = torch.ones(1,1).int().cuda()*num_verts
        initial_tensor = torch.cat([num_verts_tensor,initial_velocity.view(1,-1),v0.view(1,-1),initial_velocity.view(1,-1),v0.view(1,-1),primary_attention.view(1,-1),betas.view(1,-1)],1)
        ...
        vert_velocity_solution_fetch_indices = torch.from_numpy(np.array(range(start_ix,end_ix))).type(torch.int64).unsqueeze(0).cuda()
        fetch_indices = vert_velocity_solution_fetch_indices.repeat(len(times),1)
        velocity = torch.gather(solution,1,fetch_indices).view(-1,num_verts,3).contiguous()

        start_ix = end_ix ; end_ix = start_ix +(num_verts*3)
        vert_solution_fetch_indices = torch.from_numpy(np.array(range(start_ix,end_ix))).type(torch.int64).unsqueeze(0).cuda()
        fetch_indices = vert_solution_fetch_indices.repeat(len(times),1)
    ...
    def one_sequence(self,seq,primary_shape,cn_feat,faces,times,first_shape,poisson_solver,loss_args,train=True,test_shape=None,test_faces=None,pose=None,betas=None,full_sequence_pose=None,full_sequence_times=None,clip_num=None,split_indices=None,prev_pred_shapes=None,prev_pred_last_def=None):
        ...
        initial_velocity = primary_shape[-1] - primary_shape[0]
        ...
        pred_shapes = self.get_ode_solution_second_order(initial_velocity,first_v0,primary_shape[-1],times,betas)
        if train:
        ...
        if train:
            ...
            return seq_loss_shape,deformed_shape,v2v
        else:
            target_shape,_,_ = shape_normalization_transforms_pytorch(seq)
            v2v,_ = compute_errors(deformed_shape,target_shape,faces)
            return deformed_shape,v2v
    ...
    def amass_one_epoch(self,optimizer,epochs,loss_args,training_args,test_args,data_args,epoch):
        ...
        for bix,data in enumerate(dataloader):
            ...
            for i,(clip,times,pose) in enumerate(zip(clips,clip_times,clip_poses)):
                ...
                seq_loss,_,clip_v2v = self.one_sequence(shape_seq,primary_shape,cn_feat,faces,times,first_shape,sequence_poisson_solver,loss_args,pose=pose,betas=betas[0,0,0,:],full_sequence_pose=sequence_pose,full_sequence_times=seq_times,clip_num=i,split_indices=split_indices)
            ...
            if len(batch_codes) > 0:
                if len(batch_codes) > 1:
                    ...
                    optimizer.zero_grad() ; self.vertex_acceleration_func.zero_grad() ; self.posing_func.zero_grad() ; self.attention.zero_grad()
    ...
    def loop_epochs(self,optimizer,epochs,loss_args,training_args,test_args,data_args):
        ...
        #self.vertex_acceleration_func.load_state_dict(torch.load(training_args.weight_path+'_vertexacc'))
        best_seq_loss = 1e9
        best_v2v = 1e9
        for i in range(1000):
            ...
            if ep_seq_loss < best_seq_loss:
                ...
                torch.save(self.vertex_acceleration_func.state_dict(),training_args.weight_path+'_vertexacc')
            ...
            for bix,data in enumerate(dataloader):
                ...
                for tbix,test_data in enumerate(test_dataloader):
                    ...
                    for i,(clip,test_clip,times,source_pose) in enumerate(zip(source_clips,test_clips,source_clip_times,clip_poses)):
                        ...
                        trans_sequence,v2v_errors = self.one_sequence(clip,primary_shape,cn_feat,faces,times,rest_shape,sequence_poisson_solver,loss_args,train=False,pose=source_pose,betas=betas[0,0,0,:],full_sequence_pose=sequence_pose,full_sequence_times=sequence_times,clip_num=i,split_indices=split_indices)
