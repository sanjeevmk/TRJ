...
from init import init_jacobian_network,init_joints_encoder_network,init_ours_wks_posing_network,init_d3f_pointnet_network,init_augode_network
#from spacetime import Method
import logging
import os
from d3f_ode_train import D3FOdeTrainer
...
def run(data_args,training_args,loss_args,test_args,log_args):
   ...
   posing_network = init_ours_wks_posing_network()
   ...
   jacobian_network = init_jacobian_network(aug_dim=training_args.aug_dim)
   transformer_network = init_transformer_network(training_args.code_size)
   augode_network = init_augode_network(training_args.aug_dim)
   ...
   optimizer = init_optimizers([jacobian_network,transformer_network,posing_network,pointnet_network,augode_network],training_args.lr)

   if not test_args.test:
         method = D3FOdeTrainer(posing_network,pointnet_network,jacobian_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir,augode_network=augode_network)
         method.loop_epochs(optimizer,training_args.epochs,loss_args,training_args,test_args,data_args)
   else:
      method = D3FOdeTrainer(posing_network,pointnet_network,jacobian_network,transformer_network,joints_encoder,dataset,training_args.batch,data_args.seq_length,log_args.dir,test_dataset=test_dataset,augode_network=augode_network)
